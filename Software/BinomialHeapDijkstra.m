classdef BinomialHeapDijkstra
    ...
    methods
        ...
        function metrics = getPerformanceMetrics(obj)
            ...
            % Computational complexity: O(V * log(V) + E * log(V)) for Binomial Heap
            numNodes = length(obj.Graph);
            numEdges = sum(cellfun(@(x) size(x, 1), obj.Graph));
            complexity = sprintf('O(%d * log(%d) + %d * log(%d))', numNodes, numNodes, numEdges, numNodes);

            complexity_value = numNodes * log(numNodes) + numEdges * log(numNodes);
